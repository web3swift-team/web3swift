...
/*
    ...
    Finally adding the type specific inits to the factory routines in `EnvelopeFactory` so that objects of the new type 
...
protocol AbstractEnvelope: CustomStringConvertible { // possibly add Codable?
    ...
    ///     - when type is .transaction the transaction is encoded for hashing or transmission to the blockchain
