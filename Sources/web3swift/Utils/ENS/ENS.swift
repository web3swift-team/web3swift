...
public class ENS {
    ...
    public func getAddress(forNode node: String) async throws -> EthereumAddress {
        ...
        guard let isAddrSupports = try? await resolver.supportsInterface(interfaceID: Resolver.InterfaceName.addr.rawValue) else {
...
//        guard let isNameSupports = try? await resolver.supportsInterface(interfaceID: Resolver.InterfaceName.name.rawValue) else {
//            throw Web3Error.processingError(desc: "Resolver don't support interface with this ID")
//        }
...
//        }
    ...
    public func getContent(forNode node: String) async throws -> Data {
        ...
        guard let isContentSupports = try? await resolver.supportsInterface(interfaceID: Resolver.InterfaceName.content.rawValue) else {
            throw Web3Error.processingError(desc: "Resolver don't support interface with this ID")
        }
    ...
    public func getABI(forNode node: String, contentType: ENS.Resolver.ContentType) async throws -> (BigUInt, Data) {
        ...
        guard let isABISupports = try? await resolver.supportsInterface(interfaceID: Resolver.InterfaceName.ABI.rawValue) else {
            throw Web3Error.processingError(desc: "Resolver don't support interface with this ID")
        }
    ...
    public func getPublicKey(forNode node: String) async throws -> PublicKey {
        ...
        guard let isPKSupports = try? await resolver.supportsInterface(interfaceID: Resolver.InterfaceName.pubkey.rawValue) else {
            throw Web3Error.processingError(desc: "Resolver don't support interface with this ID")
        }
    ...
    public func getText(forNode node: String, key: String) async throws -> String {
        ...
        guard let isTextSupports = try? await resolver.supportsInterface(interfaceID: Resolver.InterfaceName.text.rawValue) else {
            throw Web3Error.processingError(desc: "Resolver don't support interface with this ID")
        }
