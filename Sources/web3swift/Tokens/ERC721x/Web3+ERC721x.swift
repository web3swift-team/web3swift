...
public class ERC721x: IERC721x {
    ...
    public func readProperties() async throws {
        ...
        guard let tokenIdPromise = try await contract.createReadOperation("tokenId", parameters: [] as [AnyObject], extraData: Data())?.call() else {return}
    ...
    func implementsERC721X() async throws -> Bool {
        transaction.callOnBlock = .latest
        let result = try await contract.createReadOperation("implementsERC721X", parameters: [] as [AnyObject], extraData: Data())!.call()
        guard let res = result["0"] as? Bool else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    func getBalance(account: EthereumAddress, tokenId: BigUInt) async throws -> BigUInt {
        transaction.callOnBlock = .latest
        let result = try await contract.createReadOperation("balanceOf", parameters: [account, tokenId] as [AnyObject], extraData: Data())!.call()
        guard let res = result["0"] as? BigUInt else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    func tokensOwned(account: EthereumAddress) async throws -> ([BigUInt], [BigUInt]) {
        transaction.callOnBlock = .latest
        let result = try await contract.createReadOperation("tokensOwned", parameters: [account] as [AnyObject], extraData: Data())!.call()
