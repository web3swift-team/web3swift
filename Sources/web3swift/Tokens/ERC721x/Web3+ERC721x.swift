...
public class ERC721x: IERC721x {
    ...
    public func readProperties() async throws {
        ...
        guard let tokenIdPromise = try await contract.createReadOperation("tokenId", parameters: [] as [AnyObject], extraData: Data())?.decodedData() else {return}
    ...
    public func getOwner(tokenId: BigUInt) async throws -> EthereumAddress {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("ownerOf", parameters: [tokenId] as [AnyObject], extraData: Data() )!.decodedData()
    ...
    public func getApproved(tokenId: BigUInt) async throws -> EthereumAddress {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("getApproved", parameters: [tokenId] as [AnyObject], extraData: Data() )!.decodedData()
        guard let res = result["0"] as? EthereumAddress else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    public func isApprovedForAll(owner: EthereumAddress, operator user: EthereumAddress) async throws -> Bool {
        ...
        let result = try await contract.createReadOperation("isApprovedForAll", parameters: [owner, user] as [AnyObject], extraData: Data() )!.decodedData()
    ...
    public func tokenByIndex(index: BigUInt) async throws -> BigUInt {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("tokenByIndex", parameters: [index] as [AnyObject], extraData: Data() )!.decodedData()
        guard let res = result["0"] as? BigUInt else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    public func tokenOfOwnerByIndex(owner: EthereumAddress, index: BigUInt) async throws -> BigUInt {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("tokenOfOwnerByIndex", parameters: [owner, index] as [AnyObject], extraData: Data() )!.decodedData()
        guard let res = result["0"] as? BigUInt else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    public func name() async throws -> String {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("name", parameters: [AnyObject](), extraData: Data() )!.decodedData()
    ...
    public func symbol() async throws -> String {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("symbol", parameters: [AnyObject](), extraData: Data() )!.decodedData()
        guard let res = result["0"] as? String else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    public func tokenURI(tokenId: BigUInt) async throws -> String {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("tokenURI", parameters: [tokenId] as [AnyObject], extraData: Data() )!.decodedData()
        guard let res = result["0"] as? String else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    func implementsERC721X() async throws -> Bool {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("implementsERC721X", parameters: [] as [AnyObject], extraData: Data() )!.decodedData()
        guard let res = result["0"] as? Bool else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    func getBalance(account: EthereumAddress, tokenId: BigUInt) async throws -> BigUInt {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("balanceOf", parameters: [account, tokenId] as [AnyObject], extraData: Data() )!.decodedData()
        guard let res = result["0"] as? BigUInt else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
    ...
    func tokensOwned(account: EthereumAddress) async throws -> ([BigUInt], [BigUInt]) {
        ...
        var transactionOptions = CodableTransaction.emptyTransaction
        transactionOptions.callOnBlock = .latest
        let result = try await contract.createReadOperation("tokensOwned", parameters: [account] as [AnyObject], extraData: Data() )!.decodedData()
