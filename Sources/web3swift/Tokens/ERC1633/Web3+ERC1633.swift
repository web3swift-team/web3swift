...
public class ERC1633: IERC1633, ERC20BaseProperties {
    ...
    func parentToken() async throws -> EthereumAddress {
        let contract = self.contract
        self.transaction.callOnBlock = .latest
        let result = try await contract.createReadOperation("parentToken", parameters: [], extraData: Data() )!.callContractMethod()
    ...
    func parentTokenId() async throws -> BigUInt {
        let contract = self.contract
        self.transaction.callOnBlock = .latest
        let result = try await contract.createReadOperation("parentTokenId", parameters: [], extraData: Data() )!.callContractMethod()
        guard let res = result["0"] as? BigUInt else {throw Web3Error.processingError(desc: "Failed to get result of expected type from the Ethereum node")}
        return res
    }
