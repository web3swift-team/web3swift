...
class web3swift_basic_local_node_Tests: XCTestCase {
    ...
    func testSignPersonal() async throws {
        let web3 = try await Web3.new(URL.init(string: "http://127.0.0.1:8545")!)
        let allAddresses = try await web3.eth.getAccounts()

        let response = try await web3.personal.signPersonalMessage(message: "hello world".data(using: .utf8)!, from: allAddresses[0])
    ...
    func testTxPoolStatus() async throws {
        let web3 = try await Web3.new(URL.init(string: "http://127.0.0.1:8545")!)
        let allAddresses = try await web3.eth.getAccounts()
        ...
        let _ = try await sendTx.sendPromise()

        let result = try await web3.txPool.getStatus()

        print(result)
    }

    func testTxPoolInspect() async throws {
        let web3 = try await Web3.new(URL.init(string: "http://127.0.0.1:8545")!)
        let allAddresses = try await web3.eth.getAccounts()
        ...
        let result = try await web3.txPool.getInspect()

        print(result)
    }

    func testTxPoolContent() async throws {
        let web3 = try await Web3.new(URL.init(string: "http://127.0.0.1:8545")!)
        let allAddresses = try await web3.eth.getAccounts()
        ...
        let result = try await web3.txPool.getContent()
