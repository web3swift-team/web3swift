...
class web3swiftKeystoresTests: XCTestCase {
    ...
    func testBIP39 () throws {
        ...
        var seed = BIP39.seedFromMmemonics(phrase.components(separatedBy: .whitespaces), password: "TREZOR")
        ...
        seed = BIP39.seedFromMmemonics(phrase.components(separatedBy: .whitespaces), password: "TREZOR")
    ...
    func testBIP39SeedAndMnemConversions() throws {
        ...
        let mnemonics = BIP39.generateMnemonicsFromEntropy(entropy: seed).components(separatedBy: .whitespaces)
    ...
    func testBIP39MnemonicIsMultipleOfThree() {
        ...
        let mnemonic_12 = "scheme spot photo card baby mountain device kick cradle pact join borrow".components(separatedBy: .whitespaces)
        ...
        let mnemonic_15 = "foster muscle start pluck when army tool surprise essay monitor impulse hello segment garage twenty".components(separatedBy: .whitespaces)
        ...
        let mnemonic_18 = "horn tenant knee talent sponsor spell gate clip pulse soap slush warm silver nephew swap uncle crack brave".components(separatedBy: .whitespaces)
        ...
        let mnemonic_21 = "weird change toe upper damp panel unaware long noise resource grant prevent file live travel price cry danger fix manage base".components(separatedBy: .whitespaces)
        ...
        let mnemonic_24 = "panda eyebrow bullet gorilla call smoke muffin taste mesh discover soft ostrich alcohol speed nation flash devote level hobby quick inner drive ghost inside".components(separatedBy: .whitespaces)
        ...
        let mnemonic_9 = "initial repeat scout eye october lucky rabbit enact unfair".components(separatedBy: .whitespaces)
        XCTAssertNil(BIP39.mnemonicsToEntropy(mnemonic_9, language: .english))

        let mnemonic_16 = "success drip spoon lunar effort unfold clinic seminar custom protect orchard correct pledge cousin slab visa".components(separatedBy: .whitespaces)
        XCTAssertNil(BIP39.mnemonicsToEntropy(mnemonic_16, language: .english))

        let mnemonic_27 = "clock venue style demise net float differ click object poet afraid october hurry organ faint inject cart trade test immense gentle speak almost rude success drip spoon".components(separatedBy: .whitespaces)
    ...
    func testNewBIP32keystore() throws {
        let mnemonic = try! BIP39.generateMnemonics(bitsOfEntropy: 256)!.components(separatedBy: .whitespaces)
    ...
    func testBIP32keystoreMatching() throws {
        let keystore = try? BIP32Keystore(mnemonics: mnemonic.components(separatedBy: .whitespaces), password: "", mnemonicsPassword: "banana")
        XCTAssertNotNil(keystore)
        let account = keystore!.addresses![0]
        let key = try! keystore!.UNSAFE_getPrivateKeyData(password: "", account: account)
    ...
    func testBIP32keystoreCustomPathMatching() throws {
        let keystore = try? BIP32Keystore(mnemonics: mnemonic.components(separatedBy: .whitespaces), password: "", mnemonicsPassword: "banana", prefixPath:"m/44'/60'/0'/0")
        XCTAssertNotNil(keystore)
        let account = keystore!.addresses![0]
        let key = try! keystore!.UNSAFE_getPrivateKeyData(password: "", account: account)
        let pubKey = Web3.Utils.privateToPublic(key, compressed: true);
